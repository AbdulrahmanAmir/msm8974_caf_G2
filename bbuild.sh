...
BOEFFLA_VERSION="1.0-CM12.1-$choice"
...
KERNEL_SAMSUNG="n"
OUTPUT_FOLDER=""
TOOLCHAIN="/home/llama/kernel/arm-eabi-4.8/bin/arm-eabi-"
...
export ARCH=arm
export CROSS_COMPILE="/home/llama/kernel/arm-eabi-4.8/bin/arm-eabi-"

BOEFFLA_DATE=$(date +%Y%m%d)


BOEFFLA_FILENAME="LLAMASWEET-kernel-$BOEFFLA_VERSION"
...
step0_copy_code()
{
	...
	sed "s/\`echo \$LINUX_COMPILE_HOST | \$UTS_TRUNCATE\`/genesixxbf3/g" -i $BUILD_PATH/scripts/mkcompile_h
}
...
step2_make_config()
{
	...
	if [ ! -z "$OUTPUT_FOLDER" ]; then
		...
		MAKESTRING="O=$OUTPUT_FOLDER $DEFCONFIG"
	fi

	# jump to build path and make config
	cd $BUILD_PATH
	echo "Makestring: $DEFCONFIG"
	make $DEFCONFIG
}
...
step3_compile()
{
	...
	# compile source
	if [ -z "$OUTPUT_FOLDER" ]; then
		make -j$NUM_CPUS  |tee ../compile.log
	else
...
step6_repack_ramdisk()
{
	...
	./tools_boeffla/mkbootfs /home/llama/kernel/msm8974_caf_G2/ramdisk_aosp/  | gzip > /home/llama/kernel/msm8974_caf_G2/ramdisk.gz
	...
	if [ "y" == "$COMPILE_DTB" ]; then
		$BUILD_PATH/tools_boeffla/mkbootimg --kernel zImage --ramdisk ramdisk.gz --cmdline "$cmdline" --base $MKBOOTIMG_BASE --pagesize $MKBOOTIMG_PAGESIZE --ramdisk_offset $MKBOOTIMG_RAMDISK_OFFSET --tags_offset $MKBOOTIMG_TAGS_OFFSET --dt dt.img -o boot.img
	else
		$BUILD_PATH/tools_boeffla/mkbootimg --kernel zImage --ramdisk ramdisk.gz --cmdline "$cmdline" --base $MKBOOTIMG_BASE --pagesize $MKBOOTIMG_PAGESIZE --ramdisk_offset $MKBOOTIMG_RAMDISK_OFFSET --tags_offset $MKBOOTIMG_TAGS_OFFSET -o boot.img
	fi
	...
	# compose updater script
		...
		echo "assert(getprop(\"ro.product.device\") == \"$choice\" ||" >> META-INF/com/google/android/updater-script
		echo "getprop(\"ro.build.product\") == \"$choice\" ||" >> META-INF/com/google/android/updater-script
